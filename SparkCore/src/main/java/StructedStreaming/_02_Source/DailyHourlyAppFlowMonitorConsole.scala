...
import StructedStreaming._02_Source.DailyHourlyAppFlowMonitorRelease.TraceEvent
...
object DailyHourlyAppFlowMonitorConsole {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val traceEvent: Dataset[TraceEvent] = spark.readStream.format("kafka")
      ...
      .map(x => {
        ...
        if(traceEvent.userid == null){
          traceEvent.userid = "null"
        }else if(traceEvent.userid ==  ""){
          traceEvent.userid = ""
        }
...
//      .option("topic", KafkaTopic.TRACE_REPORT)
//      //.option("kafka.security.protocol", "SASL_PLAINTEXT")
...
//        |     trace_event_daily t
//        |group by
//        |     t.event_day,t.date_key,t.app_id,t.__app_version
//        |""".stripMargin)
//      .toKafkaDF("__table_name")
//      .writeStream
//      .outputMode("update")
